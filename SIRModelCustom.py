...
class SIRModelCustom(DiffusionModel):
	"""
	...
	def __init__(self, graph, fixed_values = True):
		"""
			 Model Constructor
	...
	def iteration(self, node_status=True):
		...
		for u in self.graph.nodes():
			...
			if(self.fixed_values):
				...
				# self.graph.node[u]['beta'] = eventp
				# self.graph.node[u]['gamma'] = eventp

				if u_status == 0:
					infected_neighbors = len([v for v in neighbors if self.status[v] == 1])
					if self.graph.node[u]['beta'] < self.params['model']['beta'] * infected_neighbors:
						actual_status[u] = 1
				elif u_status == 1:
